...
import actio.actio.ExploreActivity;
import actio.actio.FriendsActivtiy;
import actio.actio.MapActivity;
import actio.actio.ProfileActivity;
import actio.actio.R;
import actio.actio.Settings;
...
//		setSupportProgressBarIndeterminateVisibility(false);
        ...
        logged_in = !dbl.getStuffValue("username").equals("DOES NOT EXIST");

        mDrawerTitles = new ArrayList<String>(Arrays.asList(getResources().getStringArray(R.array.drawer_array)));
        if(!logged_in) {
            mDrawerTitles.remove(3);
            mDrawerTitles.remove(3);
        }
...
//			boolean go_details = false;
			...
			if(position == 0) {
				...
				if(this_is.equals("details"))	
		        	i.putExtra("FROM_DETAILS", true);
			} else if(position == 5 || (position == 3 && !logged_in)) {
